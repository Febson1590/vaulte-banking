    ...
    <section id="features" className="features" style={{ padding: "100px 5%", background: "#06091A", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", position: "relative" }}>
        ...
        <div className="section-head" style={{ textAlign: "center", marginBottom: 64 }}>
          ...
          <p style={{ fontSize: "clamp(14px, 1.4vw, 17px)", color: "rgba(255,255,255,0.5)", maxWidth: 500, margin: "0 auto", lineHeight: 1.75 }}>
            Powerful financial tools built for the modern world. Send, save, and manage money from anywhere.
          </p>
        </div>

        {/* Cards */}
        <div className="features-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, minmax(0,1fr))", gap: 20 }}>
          {features.map((f, i) => (
            <div key={i} className="feature-card"
              style={{ padding: "32px 24px", borderRadius: 20, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", transition: "all 0.3s ease", position: "relative", overflow: "hidden", minWidth: 0, display: "flex", flexDirection: "column" }}
              onMouseEnter={e => {
            ...
            >
              ...
              <h3 style={{ fontSize: 17, fontWeight: 700, color: "#fff", marginBottom: 10, letterSpacing: "-0.2px" }}>{f.title}</h3>
              <p style={{ fontSize: 14, color: "rgba(255,255,255,0.55)", lineHeight: 1.7, margin: 0 }}>{f.description}</p>
              {/* Corner accent */}
              <div aria-hidden="true" style={{ position: "absolute", bottom: 0, right: 0, width: 80, height: 80, background: `radial-gradient(circle at bottom right, ${f.glow}, transparent)`, pointerEvents: "none" }} />
            </div>
          ))}
        </div>
      </div>
      ...
      <style>{`
        @media (max-width: 1024px) {
          .features .features-grid { grid-template-columns: repeat(2, minmax(0,1fr)) !important; gap: 18px !important; }
        }
        @media (max-width: 640px) {
          .features { padding: 72px 5% !important; }
          .features .section-head { margin-bottom: 44px !important; }
          .features .features-grid { grid-template-columns: 1fr !important; gap: 14px !important; }
          .features .feature-card { padding: 26px 22px !important; border-radius: 18px !important; }
        }
      `}</style>
    </section>
  );

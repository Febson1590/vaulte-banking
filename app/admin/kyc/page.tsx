...
export default function AdminKYC() {
  const [kycs, setKycs] = useState(initialKYC);
  const [filter, setFilter] = useState("All");
  const [selected, setSelected] = useState<typeof initialKYC[0] | null>(null);
    ...
    <AdminLayout title="KYC Management">
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "24px" }}>
        <div>
          ...
          <p style={{ margin: "4px 0 0", color: "#6B7280", fontSize: "14px" }}>{pending} submissions pending review</p>
        </div>
        <div style={{ display: "flex", gap: "10px" }}>
          {[
            { label: `Pending (${kycs.filter(k => k.status === "Pending").length})`, color: "#D97706", bg: "#FFFBEB" },
            { label: `Approved (${kycs.filter(k => k.status === "Approved").length})`, color: "#059669", bg: "#ECFDF5" },
            { label: `Rejected (${kycs.filter(k => k.status === "Rejected").length})`, color: "#DC2626", bg: "#FEF2F2" },
          ].map(badge => (
            <div key={badge.label} style={{ background: badge.bg, color: badge.color, borderRadius: "8px", padding: "8px 14px", fontSize: "13px", fontWeight: 700 }}>{badge.label}</div>
          ))}
        </div>
      </div>
              ...
              <tr key={kyc.id} style={{ borderTop: "1px solid #F3F4F6", background: i % 2 === 0 ? "#fff" : "#FAFAFA" }}>
                <td style={{ padding: "14px 16px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    <div style={{ width: "34px", height: "34px", borderRadius: "50%", background: "#EEF4FF", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, color: "#1A73E8", fontSize: "13px", flexShrink: 0 }}>
                      {kyc.user.split(" ").map(n => n[0]).join("").slice(0, 2)}
                    </div>
                    <span style={{ fontSize: "14px", fontWeight: 600, color: "#0A1628" }}>{kyc.user}</span>
                  </div>
                </td>
